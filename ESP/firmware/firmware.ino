...
void setup() {
  ...
  // Init LittleFS
  // if(!LittleFS.begin()) {
  //   Serial.println("LittleFS mount failed — files won't be available");
  // } else {
  //   Serial.println("LittleFS mounted");
  // }
  if (!LittleFS.begin()) {  // 'true' forces format if mount fails
...
void loop() {
  ...
  if (wifiMulti.run(connectTimeoutMs) == WL_CONNECTED) {
    ...
    if (httpCode > 0 && httpCode2 > 0) {
      ...
      if (!err1) {
        // Fill RELAY_STAT from JSON array
    ...
    if (isnan(h) || isnan(t)) {
      ...
      Serial.print("%  Inside Temperature: "); Serial.print(t);
      Serial.print("°C  Outside Temperature: "); Serial.print(outsideTemp);
      Serial.println("°C");
    }

    // Periodic OTA check (non-forced). Frequency: every 1 hour (adjust if needed)
    ...
    // Disconnected branch — set safe state
    digitalWrite(wifiStatus, LOW);
